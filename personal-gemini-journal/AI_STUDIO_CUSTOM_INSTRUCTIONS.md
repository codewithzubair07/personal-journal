...
**Important:** Google published the official, authoritative version of these
directives in the ideathon's reference codelab — use that exact text, not a
paraphrase, since judges calibrate against it:
https://codelabs.developers.google.com/codelabs/cloud-run/cloud-run-ai-challenge#2
("Step 2: Add Custom Instructions"). Open that page and copy the
"Production Directives" block directly into Google AI Studio's Custom
Instructions field before generating any code.

That official block covers, in more depth than the summary below: agentic
threat modeling, OWASP-aligned secure coding, Firestore/Firebase Auth
hardening (including **preferring federated sign-in over email/password**),
zero-hardcoding secret management, and a security-reviewer persona — plus
two things that are easy to miss if you only skim a summary:

1. **A resilient Gemini model fallback ladder.** Never call a single
   hardcoded model. Wrap every `generateContent` call so that on a
   recoverable error (503, 429, 404, 500) it retries through a ladder of
   backup models before failing. See `lib/gemini.ts` for the implementation
   (`generateWithFallback`).
2. **Never silently drop a failed save.** If a user's input was received
   but persistence to Firestore fails, don't clear their input or fail
   silently — show a clear error with a "Retry Save" option. See
   `components/JournalChat.tsx`.

## Summary of what this app implements (for quick reference)

The paraphrased version below is what was actually used to guide this
project's build — paste the official codelab text into AI Studio itself.
