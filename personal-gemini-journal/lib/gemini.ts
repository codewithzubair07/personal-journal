...
// Resilient model fallback ladder: never depend on a single model string.
// If the primary model is unavailable/overloaded/renamed, retry through
// progressively more conservative fallbacks before surfacing an error.
const MODEL_LADDER = [
  "gemini-3.6-flash",
...
const RECOVERABLE_STATUS = new Set([404, 429, 500, 503]);
...
/**
 * Runs `fn(model)` against each model in MODEL_LADDER in order, moving to
 * the next one only on a recoverable error (rate limit, unavailable,
 * not found, internal). Throws the last error if every model fails.
 */
async function generateWithFallback<T>(
  ...
  for (const model of MODEL_LADDER) {
    try {
      ...
      if (status && !RECOVERABLE_STATUS.has(status)) {
        throw err; // non-recoverable (e.g. bad request) — don't keep retrying
      }
  ...
  throw lastErr instanceof Error
    ? lastErr
    : new Error("All Gemini models in the fallback ladder failed.");
}
...
export async function summarizeAndTag(
  ...
  const raw = await generateWithFallback(async (model) => {
